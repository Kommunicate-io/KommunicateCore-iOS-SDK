...
- (void)internalSync {
    if (self.managedObjectContext.hasChanges) {
        ALSLog(ALLoggerSeverityInfo, @"[MQTTPersistence] pre-sync: i%lu u%lu d%lu",
        ...
        ALSLog(ALLoggerSeverityInfo, @"[MQTTPersistence] postsync: i%lu u%lu d%lu",
                     (unsigned long)self.managedObjectContext.insertedObjects.count,
                     (unsigned long)self.managedObjectContext.updatedObjects.count,
                     (unsigned long)self.managedObjectContext.deletedObjects.count
                     );
...
- (MQTTCoreDataFlow *)flowforClientId:(NSString *)clientId
    ...
    ALSLog(ALLoggerSeverityInfo, @"flowforClientId requestingPerform");
    if ([NSThread isMainThread]) {
    ...
    ALSLog(ALLoggerSeverityInfo, @"flowforClientId performed");
    return flow;
}
...
- (MQTTCoreDataFlow *)internalFlowForClientId:(NSString *)clientId
    ...
    ALSLog(ALLoggerSeverityInfo, @"flowforClientId performing");
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    @synchronized (lock) {
        ...
        ALSLog(ALLoggerSeverityInfo, @"[MQTTPersistence] Persistent store: %@", persistentStoreURL.path);
...
- (void)sizes {
    ...
    ALSLog(ALLoggerSeverityInfo, @"[MQTTPersistence] sizes %llu/%llu", fileSize, fileSystemFreeSize);
}
@end
